    ...
    private function connect(){
		$this->db_connect_id = mysqli_connect($this->obj['dbhost'],$this->obj['dbuser'],$this->obj['dbpwd'],$this->obj['dbname']);
             if (!$this->db_connect_id)
              {
                echo (" Error connecting to the database server:".mysqli_connect_error());
    	  }
  }
...
function execute($query) {       
        ...
        if(!$this->q_id ) {
            ...
            die ("ERROR: error DB.<br> Cannot execute query against the database:<br> $query <br>MySql Type of Error: $error1");
            exit;
        }         
	$this->query_count++;  
	return $this->q_id;    
